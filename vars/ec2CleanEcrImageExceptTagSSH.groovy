#!/usr/bin/env groovy
//
// ec2CleanEcrImageExceptTagSSH.groovy
//
def call() {
    ...
    withCredentials( [
        ...
        sshagent(['app-ec2-ssh-key']) {
            sh '''
                set -euo pipefail
                echo "Cleaning ${ECR_REGISTRY}/${APP_IMAGE_NAME} except ${APP_IMAGE_TAG} on EC2"
...
# On EC2 :
set -euo pipefail
echo "Cleaning $ECR_REGISTRY/$APP_IMAGE_NAME except $APP_IMAGE_TAG"
